...
function fileToModels(path, skinName = null) {
...
function fileArrayToModels(paths, getSkinName = () => null) {
...
async function getSkinManifest() {
  const skinPaths = await globby(`${T2_SKINS_PATH}/docs/skins/**/*.png`);
  ...
  return {
    customSkins: [...models, ...weaponModels, ...vehicleModels].reduce(
      (skins, name, i) => {
    ...
    newSkins: [...models, ...weaponModels, ...vehicleModels].reduce(
      (skins, name, i) => {
        const modelSkins = foundModels.get(name) ?? new Map();
        skins[name] = orderBy(
          [...modelSkins.keys()].filter(
